...
def show_checkout_page(db):
    ...
    # البحث - Now added at the top
    col_search1, col_search2 = st.columns(2)
    search_patient = col_search1.text_input("🔍 ابحث باسم المريض")
    search_code = col_search2.text_input("🔍 ابحث بالكود")
    ...
    for _, row in results.iterrows():
        with st.container(border=True):
            ...
            # Display doctor or dental center
            if row['dental_center']:
                c1.markdown(f"**المركز:** {row['dental_center']}")
                if row['branch_name']:
                    c1.markdown(f"**الفرع:** {row['branch_name']}")
            else:
